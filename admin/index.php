...
// --- VERIFICAÇÃO DE PERMISSÕES DO ARQUIVO ---
if (!file_exists($settings_file)) {
    ...
    if (@file_put_contents($settings_file, $default_settings) === false) {
        $error_message = "<strong>Erro Crítico:</strong> O arquivo <code>settings.json</code> não existe e não pôde ser criado. Verifique as permissões de escrita na pasta <code>config</code>.";
    }
} elseif (!is_readable($settings_file) || !is_writable($settings_file)) {
    $error_message = "<strong>Erro Crítico:</strong> O arquivo <code>settings.json</code> não pode ser lido ou escrito. Verifique as permissões do arquivo (tente <code>chmod 664</code>).";
}
...
// LÓGICA PARA LER AS CONFIGURAÇÕES ATUAIS
$settings = [];
...
<head>
    ...
    <style>
        ...
        input[type="text"], select { width: 100%; padding: 10px; border-radius: 5px; border: 1px solid #ccc; font-size: 1em; box-sizing: border-box; }
